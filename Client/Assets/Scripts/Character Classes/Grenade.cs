...
//			if(gameObject.GetComponent<AudioSource>().isPlaying==false){
	...
	public static void fireGrenade(PangoWorld _world,Vector3 position,Vector3 direction){
		...
		GameObject  grenade;
		//if(uLink.Network.isServer||(uLink.Network.isClient && uLink.Network.status == uLink.NetworkStatus.Connected))
		//	grenade=(GameObject)uLink.Network.Instantiate(Resources.Load("grenadePrefab"),launchPos,Quaternion.identity,0);
		//else
			grenade=(GameObject)GameObject.Instantiate(Resources.Load("grenadePrefab"),launchPos,Quaternion.identity);
