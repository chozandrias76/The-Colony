...
///     The basic idea is that the server simulates all physics including gravity and checks if any player tries to cheat
///     by
///     sending movment orders as an RPC (The RPC name is ServerMove) with false coordinates to move faster than allowed in
///     the game.
...
///     AdjustOwnerPos. If the position is good, the server sends an RPC named GoodOwnerPos. They are both sent as
///     unreliable
///     RPCs from the server to the client to minimize server resources.
///     This script component also makes sure interpolation and extrapolation is used for the state synchronozation sent
///     from
///     the server to clients. The state synchronization, arriving at the client, is stored in an internal array and the
///     public properties interpolationBackTime and extrapolationLimit can be used to tune the correct behavior for every
///     game.
///     Please read the code for more details.
/// </remarks>
[AddComponentMenu("uLink Utilities/Strict Platformer")]
[RequireComponent(typeof (uLinkNetworkView))]
public class uLinkStrictPlatformer : MonoBehaviour
{
    private readonly List<Move> ownerMoves = new List<Move>();
    private readonly State[] proxyStates = new State[20];
    ...
    private bool ownerInputJumping;

    private float ownerInputVelX;
    private float ownerInputVelZ;
    private int proxyStateCount;
    private bool serverHasLastOwnerPos;
    ...
    private void uLink_OnSerializeNetworkView(BitStream stream, NetworkMessageInfo info)
    {
        ...
        else
        {
            ...
            if (proxyStates[0].timestamp < proxyStates[1].timestamp)
                Debug.LogError("Timestamp inconsistent: " + proxyStates[0].timestamp + " should be greater than " +
                               proxyStates[1].timestamp);
        }
    }
    ...
    private void Update()
    {
        if (Network.isAuthoritativeServer && Network.isServerOrCellServer)
            return;

        if (Network.isAuthoritativeServer && networkView.isMine)
        {
            ...
            networkView.UnreliableRPC("ServerMove", NetworkPlayer.server, transform.position, move.vel.x, move.vel.z,
                jumping, transform.rotation.eulerAngles.y);
    ...
    {
        if (!Network.isAuthoritativeServer || networkView.owner == NetworkPlayer.server || Network.isClient)
            return;
        ...
        if (serverHasLastOwnerPos && (Network.isCellServer || networkView.owner.isConnected))
        {
            ...
            {
                networkView.UnreliableRPC("AdjustOwnerPos", RPCMode.Owner, serverPos);
            }
            else
            {
                networkView.UnreliableRPC("GoodOwnerPos", RPCMode.Owner);
            }
        }
    }
    ...
    [RPC]
    private void ServerMove(Vector3 ownerPos, float velX, float velZ, bool jumping, float rotY, NetworkMessageInfo info)
    {
        ...
        var deltaTime = (float) (info.timestamp - serverLastTimestamp);
        var deltaPos = new Vector3(velX*deltaTime, 0, velZ*deltaTime);
    ...
    [RPC]
    private void GoodOwnerPos(NetworkMessageInfo info)
    {
        Move goodMove;
    ...
    [RPC]
    private void AdjustOwnerPos(Vector3 pos, NetworkMessageInfo info)
    {
        GoodOwnerPos(info);
