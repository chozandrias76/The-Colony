...
public class uLinkStatisticsGUI : MonoBehaviour
{
    ...
    public static void DrawGUI(Position position, bool showDetails)
    {
        ...
        GUILayout.Label(NetworkUtility.GetStatusString(Network.peerType, Network.status));
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Last Error:", GUILayout.Width(COLUMN_WIDTH));
        GUILayout.Label(NetworkUtility.GetErrorString(Network.lastError));
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Network Time:", GUILayout.Width(COLUMN_WIDTH));
        GUILayout.Label(Network.time.ToString(CultureInfo.InvariantCulture) + " s");
        GUILayout.EndHorizontal();
        ...
        if (showDetails)
        {
            ...
            GUILayout.Label((Network.config.serverTimeOffsetInMillis*0.001).ToString(CultureInfo.InvariantCulture) +
                            " s");
            GUILayout.EndHorizontal();
        }
        ...
        GUILayout.Label(Network.networkViewCount.ToString(CultureInfo.InvariantCulture));
        GUILayout.EndHorizontal();
        ...
        if (Network.isServer)
        {
            ...
            GUILayout.Label(MasterServer.isRegistered ? MasterServer.gameName : "Not Registered");
            GUILayout.EndHorizontal();
        }
        ...
        {
            NetworkStatistics stats = player.statistics;
