...
public class uLinkStrictCharacter : MonoBehaviour
{
    ...
    {
        if (!Network.isAuthoritativeServer || Network.isServerOrCellServer || !networkView.isMine)
        {
            return;
        }
        ...
        move.deltaTime = (ownerMoves.Count > 0)
            ? (float) (move.timestamp - ownerMoves[ownerMoves.Count - 1].timestamp)
            : 0.0f;
        move.vel = character.velocity;

        ownerMoves.Add(move);

        networkView.UnreliableRPC("ServerMove", NetworkPlayer.server, transform.position, move.vel, transform.rotation);
    }

    [RPC]
    private void ServerMove(Vector3 ownerPos, Vector3 vel, Quaternion rot, NetworkMessageInfo info)
    {
