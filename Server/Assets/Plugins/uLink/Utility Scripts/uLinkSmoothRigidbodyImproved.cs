...
///     Is Kinematic = false (must be set in proxy and owner - because this script does set rigidbody.velocity directly to
///     move the object in the client)
...
///     outside of this script. The movement could be controlled by AI logic or path finding logic or RPCs from some client
///     adding
///     forces to the creater object. The server does all the collision detection (using the physics engine in Unity) and
///     then the server
...
///     LAST arriving state sync message. This way the client side object will always try to move in the direction of the
///     most
...
///     using RPCs. The creator object in the server recieves this. The creator object will move due to the client input
///     and other
///     forces like objects colliding. The resulting position and rotation is send from the server to all clients using
///     state sync.
///     This script does state sync sending. The clients receives the state sync (done by this script) and calcualtes a
///     smooth
...
public class uLinkSmoothRigidbodyImproved : MonoBehaviour
{
    ...
    // This value is only used in the proxy and owner, not creator
    private readonly State mostCurrentReceivedState = new State();
    ...
    private void uLink_OnSerializeNetworkViewOwner(BitStream stream, NetworkMessageInfo info)
    {
    ...
    private void UpdateState()
    {
        ...
        var timeLeftToTarget = (float) (mostCurrentReceivedState.timestamp - Network.time);
        ...
        else if (timeLeftToTarget <= 0 &&
                 (maxExtrapolationTimeMs/1000) > (Network.time - mostCurrentReceivedState.timestamp))
        {
