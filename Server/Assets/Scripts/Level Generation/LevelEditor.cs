...
{
	//This class is used by the admin to create a map for players
	//This class uses a gui and assets that are in the local directory
    private const int EDITOR_WINDOW_ID = 1;
    private const int TILE_SIZE = 5;
    private readonly string[] editorButtonItems = new string[64];
    public string _data;
    private bool _displayEditorWindow;
    ...
    //private Vector3 currentLoadPosition;
    ...
    {
        path = path.Substring((Application.dataPath + "/Resources/Prefabs/").Length,
            path.Length - (Application.dataPath + "/Resources/Prefabs/").Length);
    ...
    private void OnGUI()
    {
        ...
        {
            scrollPosition = GUILayout.BeginScrollView(scrollPosition, GUILayout.Width(_editorScaleX + 25),
                GUILayout.Height(_editorScaleY));
        ...
        for (int y = 0; y < _editorRows; y++)
        {
            for (int x = 0; x < _editorCols; x++)
            {
                    ...
                    editorButtonItems[cnt]))
                {
                    ...
                    cursor =
                        (GameObject)
                            Instantiate(Resources.Load("Prefabs/" + selEditorButtonItem), new Vector3(x, 0, y),
                                Quaternion.identity);

                    if (cursor.name.Contains("Tile"))
                    {
                        cursor.transform.Rotate(new Vector3(270, 0, 0));
                    }
                ...
                if (y == 0 && x == 0)
                {
                    ...
                    {
                        //SaveGameMap();
                        GameObject.Find("_gameLogic").GetComponent<PopulateWorldObjects>().GetData();
                    }
                    ...
                    if (GUI.Button(new Rect(5 + (4*buttonWidth), 20 + buttonHeight, buttonWidth, buttonHeight),
                        "Initilize" + '\n' + "World"))
                    {
					}
                }
    ...
    public void AddGameObject(float addX, float addY)
    {
        ...
        go =
            (GameObject)
                Instantiate(Resources.Load("Prefabs/" + selEditorButtonItem), new Vector3(addX, 0, addY),
                    Quaternion.identity);
        go.name = selEditorButtonItem + "(" + go.transform.position.x/TILE_SIZE + "," +
                  go.transform.position.z/TILE_SIZE + "," + go.transform.position.y/TILE_SIZE + ")";
